...
import { validateEmail, validateName, validatePassword } from '../../helpers/validateData.js'
...
export const createNewUser = async (req, res) => {
  ...
  try{
    ...
    if(user) return res.status(409).json({ success: false, message : 'Usuário já existe!'})
...
export const loginUser = async (req, res) => {
  ...
  try{
    ...
    if(!isPasswordValid) return res.status(401).json({ success: false, message : 'Email ou senha inválidos'})
...
export const updateProfile = async (req, res) => {
  if(!req.body){
    return res.status(400).json({success: false, message: 'Erro na requisição!' })
  }
  ...
  try{
    ...
    if(!avatar){
      user = await user.updateOne({name, bio}, {new: true})
    }else{
      const upload = await cloudinary.uploader.upload(avatar)
      user = await user.updateOne({name, bio, avatar: upload.secure_url}, {new: true})
    }
  }catch(error){
